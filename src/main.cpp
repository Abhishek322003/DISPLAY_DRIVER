...
static const char *TAG = "EV_CHARGER_THREAD";

#define START_STOP_ADDR      0x1000
#define VOLTAGE_ADDR         0x4000
#define CURRENT_ADDR         0x4100
#define INPUT_SUPPLY_FAULT           0x4200
#define CHARGING_VOLT        0x4300
#define SOC_ADDR             0x4400
#define CHARGING_CURRENT     0x4500
...
#define CAN_ID_DISPLAY       0x100
#define CAN_ID_CHARGER       0x100
...
// -------------------- State/Globals ------------------------
volatile uint32_t GUN_CONNECTED      = 0;
volatile uint32_t READY_FOR_CHARGE   = 0;
volatile uint32_t CHARGE_ACTIVE      = 0;
volatile uint32_t EMERGENCY_STOP     = 0;
volatile uint8_t  DISPLAY_START_STOP = 0;
volatile float volts_cp = 1.8f;
static float new_val = 1.8f;
static meter_data_t g_meter;
int gun_tem  =0;
uint16_t EV_BATTERY_UN_V  =215;
uint16_t CHARGING_OVER_CURRENT=100;
uint16_t INPUT_SUP_FAULT=380;
...
static void push_meter_to_dwin(const meter_data_t *m)
{
    if (DISPLAY_START_STOP == 0 || EMERGENCY_STOP) {
        ...
        dwin_can_write(PAGE_SWITCH_ADDR, 0x0001);   // Charge complete page
        dwin_can_write(PAGE_SWITCH_INITADDR, 0x5A01);
        return;
    }
    else if(gun_tem>=10 || EV_BATTERY_UN_V<=200 ||CHARGING_OVER_CURRENT>=120||INPUT_SUP_FAULT>=410){
		//dwin_can_write(PAGE_SWITCH_ADDR, 0x000A);   // Unplugged
        //dwin_can_write(PAGE_SWITCH_INITADDR, 0x5A01);
        //vTaskDelay(pdMS_TO_TICKS(2000));
        return ;
	}
    ...
    static uint16_t unit_rate = 20;
    uint16_t amt = rand()%1000;
    uint16_t delivered = (uint16_t)(m->energy_kWh * 100.0f);

    dwin_can_write(VOLTAGE_ADDR, sc+2);
    dwin_can_write(CURRENT_ADDR, sc+1);
    dwin_can_write(INPUT_SUPPLY_FAULT, INPUT_SUP_FAULT++);
    dwin_can_write(CHARGING_VOLT, EV_BATTERY_UN_V--);
    dwin_can_write(SOC_ADDR, sc);
    dwin_can_write(CHARGING_CURRENT, tc);
    ...
    dwin_can_write(DURATION, ++duration);
    dwin_can_write(GUN_TEMPERATURE, gun_tem++);
    ...
    if (sc >= 100) {
        ...
        dwin_can_write(PAGE_SWITCH_ADDR, 0x0005);
        dwin_can_write(PAGE_SWITCH_INITADDR, 0x5A01);
    }
}
...
static void read_energy_meter(meter_data_t *out){
    ...
    if (CHARGE_ACTIVE) {
        float voltage_variation = ((float)(rand() % 40) - 20) / 10.0f;
        float current_variation = ((float)(rand() % 30) - 15) / 10.0f;
    ...
    out->soc = (energy / 60.0f) * 100.0f;
    if (out->soc > 100.0f) out->soc = 100.0f;
    out->tempC = 30.0f + (rand() % 5);
}
...
// Meter Update Task
void meter_update_task(void *arg) {
...
// Serial Voltage Task
void serial_voltage_task(void *arg) {
    while (1) {
...
void state_machine_task(void *arg) {
    while (1) {
        if (gpio_get_level(EMERGENCY_PIN) == 0) {
            if (!EMERGENCY_STOP) {
			...
			if(gun_tem>=10){
			gun_tem=-10;
			dwin_can_write(0x0085, 0x000A);
            dwin_can_write(0x0084, 0x5A01);
            ESP_LOGE(TAG, "GUN_TEMPERATURE EXCEEDED! Stopping charge immediately.");
            }
            else if(EV_BATTERY_UN_V<=200){
			EV_BATTERY_UN_V=240;
			dwin_can_write(0x0085, 0x000C);
            dwin_can_write(0x0084, 0x5A01);
            ESP_LOGE(TAG, "EV_BATTERY UNDER VOLTAGE 200! Stopping charge immediately.");
            }
            else if(CHARGING_OVER_CURRENT>=120){
			CHARGING_OVER_CURRENT=100;
			dwin_can_write(0x0085, 0x0008);
            dwin_can_write(0x0084, 0x5A01);
            ESP_LOGE(TAG, "EV_BATTERY OVER CURRENT 120! Stopping charge immediately.");
			}
			else {
				INPUT_SUP_FAULT=380;
			dwin_can_write(0x0085, 0x0009);
            dwin_can_write(0x0084, 0x5A01);
            ESP_LOGE(TAG, "Input supply fault! Stopping charge immediately.");
            }
            vTaskDelay(pdMS_TO_TICKS(2000));
            CHARGE_ACTIVE = 0;
            DISPLAY_START_STOP = 0;
            ...
            DISPLAY_START_STOP = 0;
            READY_FOR_CHARGE = 0;
            ESP_LOGE(TAG, "EV_BATTERY VOLTAGE EXCEEDED! Stopping charge immediately.");
		}
...
#define EARTH_FAULT_PIN  GPIO_NUM_15


// ---------------------- MAIN ENTRY ---------------------
void app_main(void)
{
    ESP_LOGI(TAG, "EV Charger (Threaded)");
    can_init();
    ...
    if(EARTH_FAULT_PIN){
            dwin_can_write(PAGE_SWITCH_ADDR, 0x000D);      // Earth fault page
            dwin_can_write(PAGE_SWITCH_INITADDR, 0x5A01);
            // Keep fault active for a while
            vTaskDelay(pdMS_TO_TICKS(5000)); // 5 sec fault display
            ESP_LOGI(TAG, "✅ EARTH FAULT CLEARED (Auto Recovery)");
            }
            // --- Return to Idle Page ---
            dwin_can_write(PAGE_SWITCH_ADDR, 0x0001);      
            dwin_can_write(PAGE_SWITCH_INITADDR, 0x5A01);
    ...
    xTaskCreate(meter_update_task,  "meter_update_task",   4096, NULL, 9,  NULL);
    xTaskCreate(serial_voltage_task,"serial_voltage_task", 4096, NULL, 8,  NULL);
    xTaskCreate(state_machine_task, "state_machine_task",  4096, NULL, 7,  NULL);
